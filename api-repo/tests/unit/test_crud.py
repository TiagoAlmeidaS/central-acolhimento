...
def test_list_contatos_with_filter(db):
    ...
    ContatoRepository.create(
        db, {"nome": "User 1", "telefone": "11-1111-1111", "motivo": "apoio emocional"}
    )
    ContatoRepository.create(
        db,
        {"nome": "User 2", "telefone": "11-2222-2222", "motivo": "orientação jurídica"},
    )
...
def test_count_contatos(db):
    ...
    for i in range(3):
        ContatoRepository.create(
            db, {"nome": f"User {i}", "telefone": f"11-9999-{1000+i}", "motivo": "test"}
        )
